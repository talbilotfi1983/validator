...
public class Validator {
    ...
    final Set<String> processedFilenameCollection = new HashSet<>();
        ...
        if (!config.printHelp().execute()) {
            ...
            System.err.println("------------- Debut de chargement Du fichier Zip Via le net -------------");


            config.downloadArchiveFromNetwork().execute();
            System.err.println("------------- Fin du chargement du fichier zip-------------");

            //decompression du fichier input.zip dans le dossier input et effacement du dossier input
            //si des fichiers existe deja
    ...
    TooManyValidationErrorException e) {
